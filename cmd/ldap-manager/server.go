...
func newLDAPManager(ctx *cli.Context) ldapmanager.LDAPManager {
	...
	config := ldapconfig.Config{
		...
		AdminCustomDN:       ctx.String(flags.LdapAdminCustomDN.Name),
		...
		ConfigUsername:      ctx.String(flags.LdapReadOnlyUsername.Name),
	...
	return ldapmanager.LDAPManager{
		...
		DefaultAdminCustomDN:     ctx.String(flags.DefaultAdminCustomDN.Name),
